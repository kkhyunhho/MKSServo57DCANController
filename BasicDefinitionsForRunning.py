...
class MKSServo:
    ...
    def send_motor(self, can_id, cmd_code, *data_values):
        ...
        time.sleep(0.05)    # Wait for Immediate Response
    ...
    def wait_homing(self, can_id, timeout=30):
        print("[WAIT] Waiting for Homing to Complete...")
        start = time.time()

        while time.time() - start < timeout:
            self.dev.purge(1)
            motor_data, dlc = self.create_motor_packet(can_id, 0x3B)
            self.send_can_message(can_id, motor_data, dlc)
            time.sleep(0.2)     # Homing Polling Interval (Slow Operation)
            response = self.dev.read(18)
            ...
            if len(response) == 18:
                ...
                if status2 == 0x01:
                    ...
                    return True
                if status2 == 0x02:
                    print("[FAIL] Homing Failed.")
                    return False
        print("[TIMEOUT] Homing Did Not Complete.")
        return False

    def send_motor_verified(self, can_id, cmd_code, *data_values):
        self.send_motor(can_id, cmd_code, *data_values)

        if cmd_code not in (0xF6, 0xFD, 0xFE, 0xF4, 0xF5):
            return

        time.sleep(0.1)     # Wait for Motor to Start Moving
        self.dev.purge(1)
        motor_data, dlc = self.create_motor_packet(can_id, 0xF1)
        self.send_can_message(can_id, motor_data, dlc)
        time.sleep(0.05)    # Wait for F1H Response
        response = self.dev.read(18)

        if len(response) == 18 and response[9] == 0x01:
            print("[RETRY] Motor Not Moving. Resending...")
            self.send_motor(can_id, cmd_code, *data_values)
    ...
    def setup_motor(self, can_id):
        ...
        for name, cmd, data in setup_commands:
            ...
            self.send_can_message(can_id, motor_data, dlc)
            time.sleep(0.05)    # Wait for Setup Response
            response = self.dev.read(18)
    ...
    def interactive_menu(self):
        ...
        try:
            while True:
                ...
                if choice == "99":
                    try:
                        ...
                        if cmd_code == 0x91:
                            ...
                            self.wait_homing(id_input)
                        elif cmd_code in (0xF6, 0xFD, 0xFE, 0xF4, 0xF5):
                            self.send_motor_verified(id_input, cmd_code, *data_values)
                        else:
                            self.send_motor(id_input, cmd_code, *data_values)
                        continue
                    except ValueError:
                ...
                # 4. Sending Packet
                if cmd_code in (0xF6, 0xFD, 0xFE, 0xF4, 0xF5):
                    self.send_motor_verified(id_input, cmd_code, *data_values)
                else:
                    self.send_motor(id_input, cmd_code, *data_values)
